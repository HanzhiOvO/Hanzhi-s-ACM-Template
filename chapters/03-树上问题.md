...
>
> **关键点**：`dfn`（dfs 序，重儿子优先）、`top[u]`（u 所在重链链头）、`son[u]`（重儿子）；路径上跳时 `depth[top[u]]` 深的先跳；**区间 [dfn[top[u]], dfn[u]] 在 dfn 上连续且深度递增**；改点权/边权：边权存到子节点上（边权 -> 点权映射）。路径和 = 各链区间和相加。
...
>
> **关键点**：**点分树深度 O(log n)** 是复杂度关键；每个原点在每层重心的距离要预处理存下来；堆/ multiset 支持删除用"双堆懒删除"或 multiset + erase；点分树边权 = 原树距离。

```cpp
// 动态点分治（点开关灯，查询最近亮灯距离）结构要点：
// cfa[u]   = 点分树上 u 的父重心
// dist[u][k] = 原树上 u 到第 k 层重心的距离（预处理）
// h1[u]    = multiset：u 子树内亮灯点到 u 的距离
// h2[u]    = multiset：u 子树内亮灯点到 cfa[u] 的距离（供父层用）
// 查询最近：ans 全局堆 = h1[u] 最小值 + h2 合并；翻转点 u 时沿 cfa 链更新 O(log n) 层
```
