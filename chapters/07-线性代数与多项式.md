...
> - 需要多项式取模（poly_mod）。
>
> **关键点**：积树 = 每层节点存 `Π (x - x_i)` 的多项式；求值 = 从上到下模积树；插值 = 先求 `g = Π(x-x_i)`，用 `g'(x_i)` 作分母得权重 w，再分治 `f = f_l·prod_r + f_r·prod_l`；**注意 `poly_mod` 依赖 `poly_inv`（07.15 修复版）**；n 小（<= 2e3）直接用 07.11 朴素插值。

```cpp
// 依赖：07.5 NTT（ntt / convolution_ntt）、07.15 poly_inv（整节一起抄）
...
vector<int> poly_mod(const vector<int>& a, const vector<int>& b) {
    ...
    vector<int> qr = convolution_ntt(ra, inv_rb);   // 商反转（取前 n-m+1 项）
    qr.resize(n - m + 1);
...
// 积树：nodes[o] 存区间 [l, r] 的 Π (x - x_i)；建树 O(n log^2 n)
struct ProductTree {
    int n;                                     // 点数
    ...
    void build(int o, int l, int r) {
        if (l == r) {
            node[o] = {(NTT_MOD - xs[l]) % NTT_MOD, 1};   // (x - x_i)
            return;
        }
        int mid = (l + r) >> 1;
        build(o << 1, l, mid);
...
vector<int> poly_inv(const vector<int>& a, int n) {
    ...
    while (len < n) {
        len <<= 1;
        // ⚠️ 牛顿迭代 g = g*(2 - f*g) mod x^len：卷积度数可达 1.5len，
...
vector<int> poly_sqrt(const vector<int>& a, int n) {
    ...
    while (len < n) {
        ...
        t.resize(len);
        res.resize(len);                          // ⚠️ 必须先扩到 len，否则 res[i] 越界写（堆损坏）
        for (int i = 0; i < len; ++i)
...
// 多项式幂：a^k（k 为 i64，可为 0）。完整处理 a[0] == 0 的提因子（a = x^shift * b, b[0] != 0）
vector<int> poly_pow(const vector<int>& a, i64 k, int n) {
    ...
    vector<int> r = poly_exp(l, n);              // exp(k ln b)
