...
## 真题自查 + 2026 预测 + 递进式重构（2025.8 末）

### 近四年真题考点自查（补 2022-2024 南京等站）
- 扫描 SUA wiki 2022/2023/2024 南京站官方简要题解 PDF，逐题对照模板，补缺口：
  - **10.4.1 超现实数博弈**（2024 南京 D）：值编码 m/2^e + simplest_between + 加法比较——2000 组随机验证
  - **06.22 树拓扑序计数**（2024 南京 C）：n!/Πs_u 公式——300 组 vs 暴力枚举排列
- 11.8 真题索引表补全 2022-2024 考点映射（长链剖分树 DP、平面图对偶最大流、min-max 容斥等）

### 2026 ICPC/CCPC 网络赛预测（11.9 节）
- 按出现概率×模板完备度排序：三模 NTT、超现实数博弈、树拓扑序计数（近两年新题热点复现概率最高）、树 DP+卷积、线段树优化建图、网络流对偶、ACAM+Z、交互构造、随机化、按位贪心

### 全文递进式重构（上下贯通）
- **verify.py 升级**：支持 `// 依赖：XX.Y` 注释自动拼接前置节代码编译——承接式模板也真实编译验证
- **02.7 SegTree 改 protected 基础版** → 02.8 SegLazy 继承只写新增（懒标记域+apply/push/range）、02.9 SegAssign 两行封装（赋值=乘0+加x）——200 组对拍验证继承链
- 家族承接说明：02.10 Beats/02.11 DynSeg/02.12 SegTreeAll、02.19→02.20 莫队、02.21→02.22 可持久化、02.24→02.25 FHQ、04.15→04.16 MCMF、03.1/2→03.3 树剖、05.4→05.7 AC 自动机
- 回归测试归档：tests/test_inherit_seg.cpp、test_surreal.cpp、test_toposort_count.cpp
...
- 新增/修改模板后运行 `python verify.py` 确认 0 语法错误，再 `python build.py` 重建 PDF
- 新模板带跨节依赖时必须标注「依赖：XX 节」并同步 00.4 索引表
- 复杂模板（网络流/平衡树/多项式等）改动后跑 `tests/` 下对应回归测试
